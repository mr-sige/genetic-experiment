...
test('test getRandom', () => {
    for (let i = 0; i < 100; i++) {
        const random = genetic.getRandom(0, 20);
...
test('test createIndividual', () => {
    ...
    const max = 100;
    const length = 10;
    const individual = genetic.createIndividual(min, max, length);
...
test('test createPopulation', () => {
    ...
    const length = 10;
    const count = 100;
    const population = genetic.createPopulation(count, min, max, length);
...
test('test fitness of weak individual 1', () => {
    ...
    const target = 100;
    const fitness = genetic.fitnessOfIndividual(individual, target);
...
test('test fitnessOfPopulation', () => {
    ...
    const target = 100;
    const fitness = genetic.fitnessOfPopulation(population, target);
...
test('test mutatePopulation', () => {
    ...
    const fitnessParents = genetic.fitnessOfPopulation(population, 100);
    const mutants = genetic.mutatePopulation(population, 1);
    expect(mutants.length).toEqual(100);
    expect(mutants[0].length).toEqual(6);
    expect(mutants[9].length).toEqual(6);
    const fitnessParentsAfterMutation = genetic.fitnessOfPopulation(population, 100);
    expect(fitnessParentsAfterMutation).toEqual(fitnessParents);
    const fitnessMutants = genetic.fitnessOfPopulation(mutants, 100);
...
test('test thisIsSparta', () => {
    ...
    const sparta = genetic.thisIsSparta(population, 100, 0.2);
...
test('test selectSurvivors', () => {
    ...
    const survivors = genetic.selectSurvivors(rejects, 0.3);
...
test('test breed', () => {
    ...
    const newGeneration = genetic.breed(parents, populationCount);
    expect(newGeneration.find((item) => { return item.length === 0; })).toBeUndefined();
    expect(newGeneration.length).toEqual(populationCount);
    expect(newGeneration.find((item) => { return item.length !== 6; })).toBeUndefined();
});

test('test mutate individual', () => {
    const individual = [85,21,89,62,87,52];
    const mutant = genetic.mutateIndividual(individual);
    const ftnsofindividual = genetic.fitnessOfIndividual(individual, 100);
    const ftnsofmutant = genetic.fitnessOfIndividual(mutant, 100);
    expect(ftnsofindividual).not.toEqual(ftnsofmutant);
});
